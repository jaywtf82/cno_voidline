/**
 * meter-processor.ts - Real-time audio metering worklet
...
class MeterProcessor extends AudioWorkletProcessor {
  ...
  private bufferSize = 2400; // 50ms at 48kHz
  private frameCount = 0;
  private reportInterval = 50; // 50Hz reporting
  
  // True peak detection (simplified 2x oversampling)
  private truePeakBuffer: Float32Array = new Float32Array(256);
  private truePeakIndex = 0;
  ...
  private calculateTruePeak(samples: Float32Array): number {
  ...
  private updateBuffers(rms: number, correlation: number): void {
    // Update RMS buffer
